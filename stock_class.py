...
class stock:
    ...
    def get_month_balance(self, month, year):
        month_balance_list = list(filter(lambda x: x['date'].month == month and x['date'].year == year, self.balances))
        this_month_balances = list(map(lambda x: x['balance'], month_balance_list))
    ...
    def sum_stocks_value(self):
        batch_values = list(map(lambda x: x['quantity'] * x['price'], self.batches))
    ...
    def sell(self, date, quantity, price):
        ...
        def sell_units(self, quantity_to_be_sold, price, operation_balance):
            if quantity_to_be_sold <= 0:
                ...
                remaining_quantity_to_be_sold = quantity_to_be_sold - current_batch['quantity']
                
                price_difference = price - current_batch['price']
                
                if quantity_to_be_sold >= current_batch['quantity']:
                    self.batches.pop(0)
                    this_batch_balance = price_difference * current_batch['quantity']
                else:
                    self.batches[0]['quantity'] = self.batches[0]['quantity'] - quantity_to_be_sold
                    this_batch_balance = quantity_to_be_sold * price_difference
                    
                sell_units(self, remaining_quantity_to_be_sold, price, this_batch_balance + operation_balance)      
